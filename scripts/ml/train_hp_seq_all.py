...
def decode_ids(ids: np.ndarray) -> str:
    return "".join(IDX2CHAR.get(int(i), "") for i in ids if IDX2CHAR.get(int(i), "") != PAD_TOKEN)
...
def extract_numbers_and_slash_simple(
    ...
    slash_x_min: int,
    slash_x_max: int,
) -> Optional[Tuple[np.ndarray, np.ndarray, np.ndarray, Tuple[int, int, int, int, int, int]]]:
    """
...
def build_number_bank_and_xy(
    ...
    print(f"[aug] bank: numbers={len(number_bank)} unique; X={len(X)}; Y={len(Y)}; bad={bad_cnt}")
    if slash_template is None:
        print("[aug] WARNING: slash_template is None (не нашли ни одного слеша)")
    else:
        print(f"[aug] anchors: lx1={anchor_lx1}, rx0={anchor_rx0}, sx0={anchor_sx0}, sx1={anchor_sx1}")

    return number_bank, slash_template, X, Y, anchor_lx1, anchor_rx0, anchor_sx0, anchor_sx1
...
def _ensure_h(crop: np.ndarray, H: int) -> np.ndarray:
    return crop if crop.shape[0] == H else cv2.resize(
        crop, (int(crop.shape[1] * H / crop.shape[0]), H),
...
def build_full_augmented_dataset(
    ...
    number_bank, slash_template, X, Y, anchor_lx1, anchor_rx0, anchor_sx0, anchor_sx1 = \
        build_number_bank_and_xy(all_imgs, all_texts, slash_x_min, slash_x_max)

    if not xy_cross or not number_bank or slash_template is None \
       or anchor_lx1 is None or anchor_rx0 is None or anchor_sx0 is None or anchor_sx1 is None:
        if xy_cross:
            print("[aug] xy_cross requested, but bank/anchors incomplete -> only originals.")
    ...
    def render_pair(left_num: str, right_num: str) -> Optional[np.ndarray]:
        ...
        if slash_crop.shape[1] != sw_target:
            slash_crop = cv2.resize(slash_crop, (sw_target, H), interpolation=cv2.INTER_NEAREST)
...
def train_hp_seq_all(
    ...
    samples = parse_hp_ls_export(ls_json, project_root=project_root, textarea_name="hp_text")
    ...
    net = HudHPSeqNet(in_ch=1, img_h=img_h, img_w=img_w, max_len=max_len, dropout=dropout).to(device)
    ...
    if auto_resume and os.path.isfile(last_path):
        ...
        try:
            ...
            assert ckpt["img_h"] == img_h and ckpt["img_w"] == img_w and ckpt["max_len"] == max_len, \
                "Параметры размера изображения/длины не совпадают с чекпоинтом"
            assert ckpt["vocab"] == VOCAB and ckpt["pad_token"] == PAD_TOKEN, \
                "VOCAB/PAD в чекпоинте не совпадает"

            print(f"[i] auto-resume: эпоха {start_epoch} "
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        is_better = (seq_acc > best_seq_acc) or (seq_acc == best_seq_acc and val_loss < best_val_loss)
        if is_better:
            ...
            print(f"[i] saved best (seq_acc={best_seq_acc:.4f}, val_loss={best_val_loss:.4f}) -> {best_path}")
...
def main():
    ap = argparse.ArgumentParser("Train HP HUD OCR with X/X, X/Y, Y/Y, Y/X synthesis + auto-resume")

    ap.add_argument("--ls_json", type=str, required=True,
    ...
    ap.add_argument("--bin_thr", type=int_or_none, default=200,
