...
def _normalize_target_kind(ability_type_raw: Optional[str], extra: Dict[str, str]) -> str:
...
def _consume_stat_block(lines: List[str], i: int) -> tuple[Dict[str, str], int]:
    ...
    while i < len(lines):
        ...
        if key in {"Hero Talents Talent Tree", "Hero Talents", "Talent Tree", "Hero Attributes", "Dotabuff Plus"}:
            break
        ...
        while i < len(lines):
            ...
            if cur in {"Hero Talents Talent Tree", "Hero Talents", "Talent Tree", "Hero Attributes", "Dotabuff Plus"}:
                break
...
def _parse_abilities_page(html: str) -> List[AbilityInfo]:
    ...
    while i < len(lines):
        ...
        while i < len(lines):
            ...
            if token in {"Hero Talents Talent Tree", "Hero Talents", "Talent Tree", "Hero Attributes", "Dotabuff Plus"}:
                break
...
def _repair_build_levels(levels: Dict[str, List[int]], talents: Dict[int, str]) -> Dict[str, List[int]]:
...
def _parse_point_choices_by_level(html: str) -> Dict[str, List[str]]:
    ...
    while i < len(lines):
        ...
        if cur.startswith("Pick and Win rate change for each ability based on when the first point is spent."):
            break
...
def parse_dotabuff_hero_abilities_builds(session, hero_name: str, logger=None) -> HeroAbilitiesBuilds:
...
def parse_dotabuff_hero_abilities_from_map(
    ...
    for hero_name in hero_names:
        try:
            parsed = parse_dotabuff_hero_abilities_builds(session, hero_name, logger=logger)
...
def parse_dotabuff_hero_abilities_from_id_file(
    ...
    if not isinstance(hero_map, dict):
        raise ValueError("id_hero_to_name.json must contain a JSON object: {id: hero_name}")
