...
class _Entry:
    ...
    stop_event: Optional[
        threading.Event
    ]  # если есть — реестр сможет мягко останавливать поток
...
class ThreadRegistry:
        ...
        *,
        ...
        with self._lock:
            ...
            self._entries[name] = _Entry(
                thread=t, stop_event=stop_event, manage_stop=True
            )
        if start:
            t.start()
        return t
    ...
    def set(
        ...
        *,
        ...
        with self._lock:
            self._entries[name] = _Entry(
                thread=thread, stop_event=stop_event, manage_stop=manage_stop
            )
    ...
    def exit(self, *, timeout_per_thread: Optional[float] = None) -> None:
        ...
        for name, entry in items:
            ...
            if t.is_alive():
                t.join(
                    timeout_per_thread
                    if timeout_per_thread is not None
                    else self._default_join_timeout
                )
                if t.is_alive():
                    log.warning("Thread %r did not stop in time", name)
