...
from scripts.host.app.window_placer import WindowPlacer

try:
    from scripts.host.app.thread_registry import ThreadRegistry  # опционально
except Exception:
...
def _loc_center(
    ...
    try:
        return p.locateCenterOnScreen(
            img_path, confidence=confidence, region=region, grayscale=True
        )
    except Exception:
        return None
...
class Account:
    ...
    def clear_real_steam_auth(self, steam_path: str):
        try:
            ...
            self.logger.info(
                "Очистка реального Steam: config/loginusers + userdata — ОК"
            )
        except Exception as e:
    ...
    def clear_sandboxie_cache(self, box_id: int):
        self.logger.debug(
            f"{self.username}: clear_sandboxie_cache — пропуск (автосэндбокс)."
        )
    ...
    def _login_window_absent_stably(
        self, box_id: int, duration: int = LOGIN_GONE_GRACE_SEC
    ) -> bool:
        t0 = time.time()
    ...
    def _click_cancel_by_coords(self, hwnd: int) -> bool:
        try:
            ...
            p.leftClick()
            self.logger.info(
                f"{self.username}: нажал «Отмена» в окне {hex(hwnd)} (по координатам)."
            )
            return True
        except Exception:
            return False
    ...
    def _handle_blockers_postlogin_once(self):
        ...
        for hwnd in blockers:
            if self._click_continue_anyway_in_hwnd(
                ...
                self.logger.info(
                    f"{self.username}: подтвердил 'Продолжить' и закрыл окно Steam."
                )
        ...
        for _ in range(max_attempts):
            ...
            for pid in self._proc_tree_pids():
                try:
                    ...
                    if (pinfo.name() or "").lower() in target and (
                        match_fn is None or match_fn(pinfo)
                    ):
                        return pinfo
                except Exception:
                    continue
    ...
    def launch_until_login_window(
        ...
        *,
        ...
        try:
            ...
            self.logger.info(
                f"{self.username}: запуск Steam+Dota (PID={self._root_pid}) (автосэндбокс)."
            )
        except Exception as e:
            self.logger.error(f"{self.username}: ошибка запуска → {e}")
            return None

        steam_proc = self.wait_for_process_in_box(
            box_id, ["steam.exe"], stop_event=stop_event
        )
        ...
        while time.time() < end and not hwnd:
            ...
            hwnd = self._find_login_hwnd_in_box(
                box_id, timeout_s=1, stop_event=stop_event
            )
            if hwnd:
                break
            self._handle_blockers_prelogin_once()
        ...
        self.logger.info(
            f"{self.username}: окно входа найдено: HWND={hex(hwnd)} — готов к скану."
        )
        return hwnd

    # ---------- full restart on failed scan ----------
    def _full_restart_to_login(
        ...
        try:
            ...
            self._root_pid = int(proc.pid)
            self.logger.info(
                f"{self.username}: перезапуск Steam+Dota (PID={self._root_pid})"
            )
        except Exception as e:
        ...
        while time.time() < end and not hwnd:
            if stop_event and stop_event.is_set():
                return None
            hwnd = self._find_login_hwnd_in_box(
                self.box_id or -1, timeout_s=1, stop_event=stop_event
            )
            if hwnd:
                break
            self._handle_blockers_prelogin_once()
        ...
        self.set_status("ready")
        self.logger.info(
            f"{self.username}: новое окно входа после рестарта: HWND={hex(hwnd)}"
        )
        return hwnd
    ...
    def run_qr_scanner(self, stop_event: threading.Event) -> int:
        ...
        def _spawn_once(hwnd: int) -> int:
            ...
            self.set_status("scanning")
            self.logger.info(
                f"{self.username}: запускаю qr_loger.py с HWND={hex(hwnd)}…"
            )
            ...
            try:
                while (
                    self._qr_proc
                    and self._qr_proc.poll() is None
            ...
            rc = (
                ...
                if early_success
                    ...
                    self._qr_proc.returncode
                    if self._qr_proc and self._qr_proc.returncode is not None
                    else 1
                )
            )
            self._qr_proc = None
        ...
        while not stop_event.is_set():
            if not hwnd or not _hwnd_exists(hwnd):
                self.logger.info(
                    f"{self.username}: HWND невалиден — переобнаруживаю окно логина…"
                )
                hwnd = self._find_login_hwnd_in_box(
                    self.box_id, timeout_s=20, stop_event=stop_event
                )
                if not hwnd:
                    if self._login_window_absent_stably(
                        self.box_id, duration=max(LOGIN_GONE_GRACE_SEC, 25)
                    ):
                        self.logger.info(
                            f"{self.username}: окно логина устойчиво отсутствует — вход успешен."
                        )
    ...
    def _apply_cpu_limit(self, pid: int, percent: int):
        ...
        try:
            ...
            self.logger.debug(
                f"{self.username}: не удалось применить мягкий лимит CPU: {e}"
            )
