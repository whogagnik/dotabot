...
from scripts.host.core.config import OUT_FILE_PARSER_ITEMS
...
def api_get_json(
    ...
    raise RuntimeError(
        f"Не удалось получить JSON после {retries} попыток"
    ) from last_err
...
def item_from_dotaconstants(
    ...
    if recipe_obj is not None:
        recipe_name = (
            recipe_obj.get("dname") or recipe_obj.get("name") or f"Recipe: {title}"
        )
