...
def setup_logging(
    level_name: str = "INFO", log_file: Optional[str] = None, quiet: bool = False
):
...
# ---------- HTTP helpers ----------
def http_post_input_pb(
    url: str,
    req_msg,
    params: Optional[Dict[str, str]] = None,
    accept_proto: bool = True,
    bearer: Optional[str] = None,
) -> requests.Response:
    if not hasattr(req_msg, "SerializeToString"):
        raise TypeError("req_msg должен быть protobuf-экземпляром")
    data = {
        "input_protobuf_encoded": base64.b64encode(req_msg.SerializeToString()).decode(
            "ascii"
        )
    }
    headers = {
    ...
    return HTTPS.post(
        url, headers=headers, params=params or {}, data=data, timeout=HTTP_TIMEOUT
    )


def http_post_form(
    url: str, data: Dict[str, str], *, accept_json: bool = True
) -> requests.Response:
...
def set_tokens_in_ma(
    ma: Dict, access_token: Optional[str], refresh_token: Optional[str]
) -> Tuple[Dict, bool]:
    changed = False
...
def generate_steam_twofactor_code(
    shared_secret_b64: str, ts: Optional[int] = None
) -> str:
...
def _begin_auth_credentials_json(
    login: str, enc_pwd_b64: str, ts: int
) -> tuple[int, bytes]:
    ...
    try:
        ...
        if xres == "5":
            ...
            logging.debug(
                f"BeginAuth(JSON) неполный ответ: {body} | x-eresult={xres} — пробую form-urlencoded…"
            )
    except Exception as e:
    ...
    if not (cid2 and rid2):
        raise RuntimeError(
            f"BeginAuth(form) malformed: {json.dumps(body2)[:200]} | x-eresult={xres2}"
        )
...
def update_with_steamguard_code(
    steamid: int, client_id: int, code: str, token: Optional[str]
) -> Tuple[bool, str]:
    ...
    resp = http_post_input_pb(
        BASE_URL + "UpdateAuthSessionWithSteamGuardCode/v1/",
        req,
        params=params,
        accept_proto=True,
        ...
        if ok:
            try:
                ...
                if "access_token" in out:
                    tail.append("access_token")
                if "refresh_token" in out:
                    tail.append("refresh_token")
                if "had_remote_interaction" in out:
                    tail.append(
                        f"had_remote_interaction={out['had_remote_interaction']}"
                    )
                if "new_client_id" in out:
                    tail.append(f"new_client_id={out['new_client_id']}")
                if "new_challenge_url" in out:
                    tail.append("new_challenge_url")
...
def find_qr_in_hwnd(hwnd: int, timeout: int) -> str:
    ...
    logging.info(
        f"Сканирую QR внутри окна HWND=0x{hwnd:x} ({rect[0]},{rect[1]},{rect[2]},{rect[3]})..."
    )
    t0 = time.time()
    while time.time() - t0 < timeout:
        try:
        ...
        if codes:
            ...
            return qr
        logging.debug(
            "Пока не нашёл QR в окне... (убедитесь, что QR виден в выбранном окне)"
        )
        time.sleep(1.5)
...
def rescan_qr_client_id_if_changed(
    login_hwnd: Optional[int], cur_client_id: int, *, tries: int = 6, delay: float = 0.5
) -> int:
    if not login_hwnd:
            ...
            if codes:
                ...
                if m:
                    ...
                    if new_cid != cur_client_id:
                        logging.debug(
                            f"QR challenge обновился: {cur_client_id} → {new_cid}"
                        )
                        return new_cid
        except Exception:
            pass
...
def ensure_request_id_after_approve(
    cur_client_id: int, token: str, initial_request_id: Optional[bytes]
) -> Optional[bytes]:
...
def update_with_mobile_confirm(
    steamid: int, version: int, client_id: int, secret: bytes, token: str
) -> Tuple[bool, str, int]:
    req = pb.CAuthentication_UpdateAuthSessionWithMobileConfirmation_Request()
    req.steamid = int(steamid)
    req.version = version
    req.client_id = client_id
    req.signature = hmac.new(
        secret, struct.pack("<IQQ", version, client_id, steamid), hashlib.sha1
    ).digest()
    req.confirm = True
    req.persistence = 1
    resp = http_post_input_pb(
        BASE_URL + "UpdateAuthSessionWithMobileConfirmation/v1/",
        req,
    ...
    *,
    ...
    if poll_payload_b64:
        ...
        if rid:
            ...
            logging.debug(
                f"Использую request_id из --poll-payload: {rid.hex()} (client_id={client_id})"
            )
    ...
    def acquire_tokens_via_credentials(
        login: str, password: str, ma: dict
        ...
        logging.debug(
            f"BeginAuth(JSON): client_id={client_id_c}, request_id={request_id_c.hex()}"
        )
        try:
            code = generate_steam_twofactor_code(get_shared_secret_b64(ma))
            ok_code, dbg_code = update_with_steamguard_code(
                steamid, client_id_c, code, token=None
            )
        ...
        while time.time() - t0 < 120:
            ok, data, dbg, _ = poll_auth_status(
                client_id_c, request_id_c, debug_payload=debug_payload
            )
            logging.debug(dbg)
    ...
    def run_cycle(token: str) -> Tuple[bool, Dict]:
        ...
        if code == 401:
            logging.info(
                "GetAuthSessionInfo: 401 — получаю новый токен через JSON-credentials…"
            )
            try:
                new_at, new_rt, snap = acquire_tokens_via_credentials(
                    login, password, ma
                )
                if not new_at:
                ...
                ok_bind, request_id, dbg_bind, code = get_auth_session_info(
                    client_id, token
                )
                logging.info(dbg_bind)
                if code != 200:
        ...
        if code2 == 401:
            ...
            try:
                ...
                if not new_at:
                    return False, {
                        "error": "credentials-json failed on approve",
                        "snap": snap,
                    }
                ma2, _ = set_tokens_in_ma(ma, new_at, new_rt)
                save_mafile(mafile_path or save_to, ma2)
        ...
        logging.debug(
            f"QR poll starts: client_id={client_id}, request_id={(request_id.hex() if request_id else 'none')}"
        )
        ...
        def push_totp_now():
            ...
            if last_totp_epoch is None or epoch != last_totp_epoch:
                try:
                    ...
                    _ok1, dbg1 = update_with_steamguard_code(
                        steamid, client_id, code, token
                    )
                    logging.debug(dbg1 + " | push TOTP (bearer)")
                    _ok2, dbg2 = update_with_steamguard_code(
                        steamid, client_id, code, None
                    )
        ...
        while True:
            ...
            if login_hwnd and exit_on_window_close and not _is_hwnd_alive(login_hwnd):
                logging.info(
                    "Окно логина закрыто — завершаю успехом (finish_mode=window_closed)."
                )
            ...
            if (
                login_hwnd
                and exit_on_qr_disappear
                and (now_ts - last_qr_check_ts) >= max(0.1, qr_recheck_interval)
            ):
                ...
                if _is_qr_visible_in_hwnd(login_hwnd):
                    ...
                    qr_absent_streak += 1
                    logging.debug(
                        f"[QR] not visible streak = {qr_absent_streak}/{qr_disappear_consecutive}"
                    )
                    if qr_absent_streak >= max(1, qr_disappear_consecutive):
                        logging.info(
                            "QR исчез из окна — завершаю успехом (finish_mode=qr_disappeared)."
                        )
            ...
            if time.time() - last_progress > poll_seconds:
                logging.error(
                    f"QR-сессия: нет прогресса {poll_seconds}s. last={last_snapshot}"
                )
                if exit_on_interaction and saw_interaction:
                    return True, {
                        "finish_mode": "had_remote_interaction_timeout",
                        **last_snapshot,
                    }
                return False, last_snapshot

            # 3) poll
            ok_poll, data, dbg_poll, _ = poll_auth_status(
                client_id, request_id, debug_payload=debug_payload, bearer_token=token
            )
            ...
            if exit_on_interaction and data.get("had_remote_interaction") is True:
                logging.debug(
                    "QR-сессия: had_remote_interaction=True — успех для клиента."
                )
                return True, {"finish_mode": "qr_interaction", **last_snapshot}

            if data.get("access_token") or data.get("refresh_token"):
                ma2, _ = set_tokens_in_ma(
                    ma, data.get("access_token"), data.get("refresh_token")
                )
                save_mafile(mafile_path or save_to, ma2)
            ...
            if (
                ...
                or data.get("had_remote_interaction") is True
            ):
                last_progress = time.time()
...
def parse_cli() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Steam QR Approver (JSON credentials fallback, без protobuf refresh)"
    )
    g = p.add_mutually_exclusive_group(required=True)
    g.add_argument("--mafile", help="путь к maFile")
    g.add_argument("--mafile-json", help="путь к JSON или inline JSON строка")
    g.add_argument(
        "--stdin-mafile", action="store_true", help="читать mafile JSON из STDIN"
    )
    ...
    p.add_argument(
        "--exit-on-qr-disappear",
        action="store_true",
        help="завершить успехом, если QR исчез из окна",
    )
    p.add_argument(
        "--qr-disappear-consecutive",
        type=int,
        default=3,
...
def login():
    try:
        ...
        if not acc:
            logging.error(
                "В maFile отсутствует account_name — не могу сверить с логином."
            )
        ...
        if args.force_login or not at:
            try:
                ...
                logging.info(
                    f"BeginAuth(JSON): client_id={cid}, request_id={rid.hex()}"
                )
                try:
                    code = generate_steam_twofactor_code(get_shared_secret_b64(ma))
                    ok_code, dbg_code = update_with_steamguard_code(
                ...
                while time.time() - t0 < 120:
                    ok, data, dbg, _ = poll_auth_status(
                        cid, rid, debug_payload=args.debug_payload
                    )
                    logging.debug(dbg)
                    if data.get("access_token") or data.get("refresh_token"):
                ...
                if not extract_existing_token(ma):
                    logging.error(
                        f"Не удалось получить токены через JSON-credentials. last={last}"
                    )
                    raise SystemExit(RC_FAIL_GENERIC)
        ...
        if ok:
            ...
            if "x-eresult=5" in snap_txt or "InvalidPassword" in snap_txt:
                logging.error(
                    f"Не получилось завершить (InvalidPassword). Итоговое состояние: {snapshot}"
                )
                raise SystemExit(RC_INVALID_PASSWORD)
