...
class TowerVisibilityTracker:
    ...
    def _closest_lane(
        self, pt: Tuple[int, int], lanes: Dict[str, List[Tuple[int, int]]]
    ) -> Optional[str]:
    ...
    def _infer_tiers_by_ancient_and_lanes(
        ...
        # 1) T4
        by_anc = sorted(
            [(self._dist(p, ancient_xy), i) for i, p in enumerate(pts)],
            key=lambda x: x[0],
        )
        ...
        for lane_name in ("top", "mid", "bot"):
            ...
            if len(idxs) != 3:
                ...
                continue
            idxs.sort(
                key=lambda i: self._dist(pts[i], ancient_xy)
            )  # ближе к ancient — "внутрь"
            tiers[idxs[0]] = 3
            tiers[idxs[1]] = 2
            tiers[idxs[2]] = 1
    ...
    def _normalize_towers(self, items: List[TowerInput], side: str) -> List[TowerState]:
        ...
        return [
            TowerState(x=int(x), y=int(y), tier=int(t), lane=None)
            for (x, y, t) in parsed
        ]
    ...
    def _has_color_radius(
        self, mm_rgb: np.ndarray, cx: int, cy: int, color: Tuple[int, int, int]
    ) -> bool:
        h, w, _ = mm_rgb.shape
        r = self.radius
    ...
    def _apply_vis(
        ...
        for s in lst:
            if self._in_bounds(s.x, s.y) and self._has_color_radius(
                mm_rgb, s.x, s.y, color
            ):
                s.visible_now = True
                s.alive = True
                s.last_seen = now
    ...
    @staticmethod
    def _any_recently_seen_of_tier(
        lst: List[TowerState], tier: int, now: float, timeout_sec: float
    ) -> bool:
        for s in lst:
            if s.tier != tier:
                continue
    ...
    def tick_one(
        ...
        if side == "radiant":
            return {
                "ally": pack(self.radiant, "radiant"),
                "enemy": pack(self.dire, "dire"),
            }
        else:
            return {
                "ally": pack(self.dire, "dire"),
                "enemy": pack(self.radiant, "radiant"),
            }

    @staticmethod
    def pretty_report(
        report: Dict[str, List[Dict]], now: Optional[float] = None
    ) -> str:
        tnow = time.time() if now is None else float(now)
        ...
        def fmt_block(label: str, lst: List[Dict]) -> str:
            ...
            for s in sorted(lst, key=lambda d: (d["tier"], d["x"], d["y"])):
                ...
                lines.append(
                    f"  T{s['tier']} @({s['x']:02d},{s['y']:02d}) {state:<5}  last_seen={fmt_age(s['last_seen']):>9}  flags={fl}"
                )
            return "\n".join(lines)

        return (
            fmt_block("ally", report["ally"])
            + "\n"
...
def main():
    ...
    while True:
        ...
        vis = cv2.resize(
            cv2.cvtColor(mm_rgb, cv2.COLOR_RGB2BGR),
            (300, 300),
