...
def log_timing(level=logging.DEBUG, name: str | None = None):
    """
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.log(level, f"{log_name} took {dt_ms:.3f} ms")
...
@contextmanager
def log_step(step_name: str, level=logging.DEBUG):
    """
    Контекстный менеджер для замера внутренних этапов внутри функции.
    """
    t0 = time.perf_counter()
    try:
        yield
        ...
        dt_ms = (time.perf_counter() - t0) * 1000.0
        logger.log(level, f"{step_name} took {dt_ms:.3f} ms")
...
def _check_bar_right_side(
    ...
    logger.debug(
        f"_check_bar_right_side stats: mode={mode}, roi={_short_shape(roi)}, "
        f"brown_ratio={brown_ratio:.4f}, black_ratio={black_ratio:.4f}"
    )
...
def _estimate_hp_ratio_from_roi(
    ...
    # ============================================================
    with log_step("_estimate_hp_ratio_from_roi: prepare ranges"):
    ...
    with log_step("_estimate_hp_ratio_from_roi: build fill mask"):
    ...
    with log_step("_estimate_hp_ratio_from_roi: build bg mask"):
    ...
    with log_step("_estimate_hp_ratio_from_roi: detect bar bounds"):
    ...
    # --- 4) колонки заливки
    with log_step("_estimate_hp_ratio_from_roi: detect fill length"):
        fill_cols = mask_hp.any(axis=0)
        xs_fill = np.where(
            fill_cols & (np.arange(w) >= bar_x0) & (np.arange(w) <= bar_x1)
        )[0]
        if xs_fill.size == 0:
            if return_masks:
                return 0.0, hp_pixels, bg_pixels, mask_hp, mask_bg  # type: ignore
    ...
    logger.debug(
        f"_estimate_hp_ratio_from_roi result: roi={_short_shape(roi)}, "
        f"hp_ratio={hp_ratio:.4f}, hp_pixels={hp_pixels}, bg_pixels={bg_pixels}, "
        f"bar_w={bar_w}, hp_len={hp_len}"
    )
...
def _estimate_mana_ratio_under_hp(
    ...
    with log_step("_estimate_mana_ratio_under_hp: build mana mask"):
    ...
    with log_step("_estimate_mana_ratio_under_hp: scan mana columns"):
        mana_len = 0
        gap = 0
    ...
    logger.debug(
        f"_estimate_mana_ratio_under_hp result: roi={_short_shape(roi)}, "
        f"has_mana=True, mana_ratio={mana_ratio:.4f}"
    )
...
@log_timing(logging.DEBUG)
def _check_rect_strict(
    mask_hp: np.ndarray, min_aspect: float = 4.0, max_h: int = 6
) -> bool:
    """
    Строгая проверка:
...
def largest_solid_rect(comp_mask: np.ndarray):
    ...
    logger.debug(
        f"largest_solid_rect: mask={_short_shape(comp_mask)}, best={best}, max_area={max_area}"
    )
...
def find_hp_bars(
    ...
    with log_step("find_hp_bars: build mask", logging.DEBUG):
    ...
    with log_step("find_hp_bars: morphology", logging.DEBUG):
        if right_side_mode == "hero":
            kernel_h = np.ones((1, 3), np.uint8)
            mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernel_h, iterations=1)
        elif right_side_mode == "creep":
            # Убираем одиночный шум и короткие рваные куски,
            # но стараемся не ломать реальные узкие полоски HP.
            kernel_open = np.ones((1, 4), np.uint8)
    ...
    with log_step("find_hp_bars: iterate components", logging.DEBUG):
        for label_id in range(1, num_labels):
            ...
            roi = frame_rgb[
                y0_bar : y1_bar + 1, x0_bar : x1_bar + 1
            ]  # +1, чтобы включить нижнюю/правую границу
            if roi.size == 0:
                continue

            hp_ratio, hp_pixels, bg_pixels, mask_hp, mask_bg = (
                _estimate_hp_ratio_from_roi(
                    roi,
...
def _filter_creeps_touching_self(
    ...
    logger.debug(
        f"_filter_creeps_touching_self: input={len(creeps)}, self={len(self_bars)}, output={len(filtered)}"
    )
    return filtered
...
def find_main_hwnd_for_pid(pid: int) -> Optional[int]:
    ...
    try:
        with log_step("find_main_hwnd_for_pid: EnumWindows", logging.DEBUG):
    ...
    with log_step("find_main_hwnd_for_pid: sort candidates", logging.DEBUG):
        arr.sort(key=_window_area, reverse=True)

    logger.debug(
        f"find_main_hwnd_for_pid: pid={pid}, candidates={len(arr)}, selected={arr[0] if arr else None}"
    )
    return arr[0]
...
def grab_roi_rgb_from_window(hwnd: int):
    ...
    try:
        with log_step("grab_roi_rgb_from_window: screenshot", logging.DEBUG):
            arr = np.array(p.screenshot(region=(cx, cy, cw, ch)))
        logger.debug(
            f"grab_roi_rgb_from_window: hwnd={hwnd}, region=({cx},{cy},{cw},{ch}), frame={_short_shape(arr)}"
        )
        return arr  # RGB
    except Exception as e:
        print("[grab]", e)
        logger.exception("grab_roi_rgb_from_window failed")
        return None
