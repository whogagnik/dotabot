...
try:
    from scripts.host.ml.train_minimap_heatmap import (
        load_model,
        infer_image,
...
class TasksWriter:
    ...
    def __init__(
        ...
        pred_box_px: int = PRED_BOX_PX_DEFAULT,
    ):
    ...
    def _flush(self):
        try:
            ...
            print(
                f"[warn] ls_tasks.json replace failed: {e}. Using shadow for this iteration."
            )
    ...
    def save_sample(
        ...
        if self.dump_preds:
            ...
            task["predictions"] = [
                {"result": preds, "score": 1.0, "model_version": "minimap-auto"}
            ]
...
def main():
    ...
    ap = argparse.ArgumentParser(
        "Minimap 100x100 → ls_tasks.json (kp/image rectanglelabels)"
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=0,
        help="сдвиг X от правого-нижнего угла: влево(+)/вправо(-), px",
    )
    ap.add_argument(
        "--dy",
        type=int,
        default=0,
        help="сдвиг Y от правого-нижнего угла: вверх(+)/вниз(-), px",
    )
    ...
    ap.add_argument(
        "--auto_dump_if_miss_self",
        type=int,
        default=1 if AUTO_DUMP_IF_MISS_SELF_DEFAULT else 0,
    )
    ...
    writer = TasksWriter(
        args.dump_dir, dump_preds=bool(args.dump_preds), pred_box_px=args.pred_box_px
    )
    ...
    while True:
        ...
        cv2.putText(
            vis,
            f"FPS:{fps_ema:5.1f} thr:{thr:.2f} nms:{nms_k}",
            (4, vis.shape[0] - 6),
        ...
        if (
            args.auto_dump_if_miss_self
            and len(peaks.get(self_ch, [])) == 0
            and (now - last_dump_ts) >= 1.0
        ):
            should_dump = True
