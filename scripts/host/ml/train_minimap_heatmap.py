...
from scripts.host.ml.infer import find_peaks_per_channel
from scripts.host.ml.metrics import (
    confusion_binary,
...
def parse_ls_export(
    ...
    def _resolve_path(
        img_path: Optional[str], file_upload: Optional[str]
        ...
        def add_suffix_variants(name: str):
            ...
            if "-" in name:
                ...
                cands.extend(
                    glob.glob(os.path.join(root, "**", suffix), recursive=True)
                )
    ...
    for task in data:
        ...
        img_path = (
            data_field.get("image") or data_field.get("img") or data_field.get("path")
        )
...
class MinimapHeatmapDataset(Dataset):
    ...
    def _rotate_boxes_ccw(
        self, boxes: List[Dict[str, Any]], k: int
    ) -> List[Dict[str, Any]]:
        if k % 4 == 0:
    ...
    def _shift_img_and_boxes(
        self, img: np.ndarray, boxes: List[Dict[str, Any]], dx: int, dy: int
        ...
        for b in boxes:
            ...
            if (nx2 - nx1) < self.drop_tiny_boxes_px or (
                ny2 - ny1
            ) < self.drop_tiny_boxes_px:
                continue
    ...
    def _dump_augmented(self, img_rgb, boxes, src_path, rot_k, dx, dy):
        ...
        with open(
            os.path.join(self.debug_dump_dir, name + ".json"), "w", encoding="utf-8"
        ) as f:
    ...
    def __getitem__(self, idx: int):
        ...
        img_resized = cv2.resize(
            img, (self.out_size, self.out_size), interpolation=cv2.INTER_AREA
        )
        ...
        if (
            self.augment
            and (self.shift_max > 0)
            and (random.random() < self.shift_prob)
        ):
...
def centers_from_boxes_percent(
    boxes: List[Dict[str, float]], size: int, cls2idx: Dict[str, int]
    ...
    *,
    ...
    for x, y, metas in tqdm(loader, desc=desc):
        ...
        Ppk, Rpk = eval_peaks_batch(
            prob_np, metas, classes, thr=peaks_thr, nms_kernel=nms_kernel, radius=radius
        )
    ...
    macro = (
        {k: _mean(v) for k, v in macro_vals.items()}
        if macro_vals["f1"]
        else {k: 0.0 for k in macro_vals.keys()}
    )
...
def train(args):
    ...
    samples = parse_ls_export(
        args.ls_json, classes=classes, image_root=args.image_root, strict_exists=True
    )
    ...
    if not samples:
        ...
        print(
            f"[debug] parsed={len(dbg)}, found_on_disk={sum(int(s['exists']) for s in dbg)}"
        )
        for s in dbg[:5]:
            print(
                " image_path:",
    ...
    print(
        f"[i] split: total={n_total}  train={len(train_samples)}  val={len(val_samples)}  test={len(test_samples)}"
    )

    debug_dump_dir = (
        os.path.join(args.out_dir, "debug_dump") if args.debug_dump_aug else None
    )
    ...
    val_ds = MinimapHeatmapDataset(
        val_samples, classes, out_size=args.size, augment=False, synthetic=False
    )
    test_ds = (
        MinimapHeatmapDataset(
            test_samples, classes, out_size=args.size, augment=False, synthetic=False
        )
        if test_samples
        else None
    )
    ...
    net = ResUNetLite(
        in_ch=3, base=args.base, out_ch=len(classes), dropout=args.dropout
    ).to(device)
    ...
    if args.resume:
        ...
        if args.strict_resume:
            if (
                ...
                or ckpt_size != args.size
                or ckpt_base != args.base
            ):
                raise RuntimeError(
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        print(
            ...
            f"[VAL presence@thr={args.pres_thr:.2f}] "
            + " | ".join(val_out["presence_lines"])
        )
    ...
    if test_loader is not None:
        ...
        print(
            ...
            f"[FINAL TEST presence@thr={args.pres_thr:.2f}] "
            + " | ".join(test_out["presence_lines"])
        )
    else:
...
def build_argparser():
    ap = argparse.ArgumentParser(
        "Train multi-heatmap detector for minimap (Label Studio rectangles)"
    )
    ap.add_argument(
        "--ls_json",
        type=str,
        required=True,
        help="Путь к экспортированному JSON из Label Studio",
    )
    ap.add_argument(
        "--image_root",
        type=str,
        default=None,
        help="Корень, где лежат PNG из экспорта (без префиксов)",
    )
    ap.add_argument(
        "--out_dir",
        type=str,
        default="runs/minimap",
        help="куда писать чекпоинты и debug",
    )
    ...
    ap.add_argument(
        "--val_split", type=float, default=0.15, help="доля val ОТ ОСТАТКА (после test)"
    )
    ap.add_argument(
        "--test_split",
        type=float,
        default=0.10,
        help="доля test ОТ ВСЕГО (0 = отключить test)",
    )
    ...
    ap.add_argument(
        "--shift_max", type=int, default=50, help="dx,dy в [-shift_max, shift_max]"
    )
    ap.add_argument(
        "--shift_prob",
        type=float,
        default=1.0,
        help="вероятность применения shift на train",
    )
    ap.add_argument(
        "--shift_border_value",
        type=int,
        default=0,
        help="цвет заполнения при сдвиге (0..255)",
    )
