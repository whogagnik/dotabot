...
from scripts.host.core.config import *  # STATUS_LABELS / STATUS_COLORS и прочие константы — из общего файла
from scripts.host.core.utils import (
    _force_foreground,
    _window_area,
    find_main_hwnd_for_pid,
    _window_ok,
    steam64_to_friend_id_local,
)
...
class GameAutomation:
    ...
    def __init__(
        ...
        self.PNG = {
            ...
            # invites
            "accept_invite_ru": os.path.join(
                self.images, "lobby", "accept-invite-ru.png"
            ),
            "accept_invite_eng": os.path.join(
                self.images, "lobby", "accept-invite-eng.png"
            ),
            ...
            "lock_disabled_ru": os.path.join(
                self.images, "game", "lock-disabled-ru.png"
            ),
            ...
            # welcome/popups
            "welcome_not_new_ru": os.path.join(
                self.images, "welcome", "not_new_ru.png"
            ),
            "welcome_not_new_en": os.path.join(
                self.images, "welcome", "not_new_en.png"
            ),
    ...
    def _grab_fullscreen_rgb(self, force: bool = False):
        ...
        if (
            (not force)
            and self.last_full_rgb is not None
            and (now - self.last_full_ts) < self.full_frame_min_dt
        ):
    ...
    @staticmethod
    def _safe_crop(
        full_rgb: np.ndarray, x: int, y: int, w: int, h: int
    ) -> Optional[np.ndarray]:
    ...
    def _loc_center(
        self, hwnd, img_path, confidence: float = 0.87, delta: float = 0.01
    ):
        """
        ...
        try:
            ...
            if W <= 0 or H <= 0:
                if self.log:
                    self.log.debug(
                        f"[LOC] hwnd={hex(hwnd)} invalid client rect: {(L, T, W, H)}"
                    )
                return None
            ...
            if (not need_new) and (hwnd in self.hwnd_haystack_pil):
                ...
                if self.log:
                    self.log.debug(
                        f"[LOC] hwnd={hex(hwnd)} haystack CACHED {W}x{H} @ {L},{T}"
                    )
            else:
                ...
                if crop_rgb is None:
                    if self.log:
                        self.log.debug(
                            f"[LOC] hwnd={hex(hwnd)} crop out-of-bounds win=({L},{T},{W},{H})"
                        )
                    return None
                ...
                if self.log:
                    ...
                    self.log.debug(
                        f"[LOC] hwnd={hex(hwnd)} haystack NEW {cw}x{ch} @ {L},{T} (delta={delta:.3f}s)"
                    )
            ...
            if needle is None:
                try:
                    ...
                    if self.log:
                        self.log.debug(
                            f"[LOC] needle loaded: '{img_path}' size={needle.size}"
                        )
                except Exception as e:
                    if self.log:
            ...
            box = p.locate(
                needle, haystack_pil, confidence=float(confidence), grayscale=True
            )
            ...
            cx_local, cy_local = p.center(
                box
            )  # координаты внутри haystack (кропа окна)
            ...
            if self.log:
                self.log.error(
                    f"[LOC] _loc_center failed hwnd={hex(hwnd)}: {e}", exc_info=True
                )
            return None
    ...
    def wait_window_ready(
        ...
        if not ok:
            self.log.warning(
                f"[IMG] Window {hex(hwnd)} not ready in {timeout:.0f}s (continuing)."
            )
        return ok

    def dismiss_welcome_if_present(
        self,
        hwnd: int,
        timeout: float = 8.0,
        stop_flag: Optional[Callable[[], bool]] = None,
    ...
    def make_parties(
        ...
        if n < 5:
            self.log.info(
                "[IMG] Not enough accounts for party (<5) — skipping make_parties"
            )
            return
        ...
        if not friend_ids and not steamids64:
            self.log.warning(
                "[IMG] friend_ids/steamids64 not provided — skipping party build."
            )
            return
        ...
        for hwnd in hwnds:
            ...
            region = _window_area(hwnd)
            paths = [
                self.PNG.get("accept_invite_ru", ""),
                self.PNG.get("accept_invite_eng", ""),
            ]
    ...
    def search_games(
        ...
        if should_make_party:
            self.log.info(
                "[IMG] make_parties=True passed here — ignored; parties are built in run_with_hwnds."
            )
        ...
        for hwnd in hwnds:
            ...
            for ph in accept_paths:
                pt = self._loc_center(hwnd, ph)
                if pt:
                    self._click(hwnd, pt, delay=0.04)
                    break
    ...
    def wait_lock_enabled(
        self, hwnd: int, timeout_s: float = 30.0, poll: float = 0.2
    ) -> bool:
    ...
    def pick_hero_grid(
        ...
        *,
        ...
        for hero in list(heroes):
            ...
            if not found_icon:
                self.log.info(
                    f'[IMG] {hex(hwnd)}: hero "{hero}" unavailable (banned/picked). Next…'
                )
                continue
            ...
            if not self.wait_lock_enabled(hwnd, timeout_s=12.0):
                self.log.info(
                    f'[IMG] {hex(hwnd)}: failed to lock "{hero}" (lock not enabled?). Next…'
                )
                continue
            ...
            if pt_lock:
                ...
                return hero
            self.log.info(
                f'[IMG] {hex(hwnd)}: failed to lock "{hero}" (no button?). Next…'
            )
