...
from scripts.host.core.utils import *  # noqa
...
def _nearest_point(
    poly: List[Dict[str, float]], pt: Tuple[float, float]
...
def _closest_point_on_polyline(
    poly: List[Dict[str, float]], p: Tuple[float, float]
) -> Tuple[float, float, float]:
    """
...
class Senses:
    ...
    def to_catboost_features(self) -> Dict[str, Any]:
        return {
            ...
            "enemy_minus_ally_heroes": int(self.enemy_hero_cnt_screen)
            - int(self.ally_hero_cnt_screen),
            "enemy_minus_ally_creeps": int(self.enemy_creep_cnt_screen)
            - int(self.ally_creep_cnt_screen),
...
class Brain:
    ...
    def __init__(
        ...
        # цель для MOVING
        self._moving_point: Optional[tuple[float, float]] = (
            None  # (x,y) в координатах миникарты 0..100
        )
        self._moving_radius: float = (
            5.0  # радиус "достаточно близко" (в тех же единицах)
        )
        self.lane_offset_px: float = (
            80.0  # насколько пикселей отходить от линии по нормали
        )
        ...
        self.farm_lane_prob: float = (
            0.35  # шанс выбрать LANE_THEN_JUNGLE (если линия будет задана)
        )
        ...
        self._lane_anchor_uv: Optional[Tuple[float, float]] = (
            None  # ����� 0..100 �� ����� � �1
        )
    ...
    def tick_one(self, snap: "Snapshot"):
        ...
        if d is not None:
            ...
            if self.log:
                self.log.info(
                    f"[BRAIN {hex(self.hwnd)}] manual switch: {d} -> {new_state.name}"
                )
        else:
    ...
    def _gather_senses(self, c: Dict[str, Any]) -> Senses:
        ...
        closest_enemy_hero_hp_ratio, closest_enemy_hero_dist_px = (
            self._closest_enemy_hero_features(c)
        )
        ...
        best_enemy_lasthit_hp_ratio, best_enemy_lasthit_dist_px = (
            self._best_lasthit_features(c, enemy=True, hp_threshold=0.5)
        )
        ...
        now = time.time()
        last_action_delta = (
            (now - self.last_action_ts) if self.last_action_ts > 0 else 9999.0
        )
        lane_wave_seen_delta = (
            (now - self._lane_wave_last_seen_ts)
            if self._lane_wave_last_seen_ts > 0
            else 9999.0
        )
        ...
        return Senses(
            ...
            landmarks=(
                (c.get("landmarks") or {}).get("data")
                if isinstance(c.get("landmarks"), dict)
                else None
            ),
            ...
            farm_phase_name=(
                self._farm_phase.name.lower()
                if self._farm_phase is not None
                else "none"
            ),
            lane_farm_phase_name=(
                self._lane_farm_phase.name.lower()
                if self._lane_farm_phase is not None
                else "none"
            ),
            self_x_uv=self_x_uv,
            self_y_uv=self_y_uv,
    ...
    def _compute_arc_waypoint_to_fountain(
        ...
        wx = (
            cx + ux * self.retreat_arc_forward_uv + nx * sign * self.retreat_arc_side_uv
        )
        wy = (
            cy + uy * self.retreat_arc_forward_uv + ny * sign * self.retreat_arc_side_uv
        )
    ...
    def _compute_dist_to_enemy_hero(self, s: Senses) -> float:
        ...
        dist = (
            self.min_enemy_dist_px
            + (self.max_enemy_dist_px - self.min_enemy_dist_px) * r
        )
        return dist
    ...
    def _flush_catboost_dataset_to_csv(self, force: bool = False) -> None:
        ...
        if (not force) and (
            len(self._catboost_dataset_rows) < self.catboost_dataset_flush_every
        ):
            return
        ...
        for key, kind in key_to_kind.items():
            ...
            for pt in points_raw:
                try:
                    ...
                    continue
                camps.append(
                    CampNode(camp_id=cid, kind=kind, x=x, y=y, state=CampState.READY)
                )
                cid += 1
    ...
    def _minimap_click_throttled(
        ...
        now = time.time()
        cd = (
            self._farming_minimap_click_cooldown
            if cooldown is None
            else float(cooldown)
        )
    ...
    @debug_log_result
    def _compute_laning_point(
        self, c: Dict[str, Any], min_dist_to_enemy: float
        ...
        ally_pts = np.array(
            [self._hpbar_center(cb) for cb in ally_creeps], dtype=np.float32
        )
        ...
        if len(enemy_creeps) >= 1:
            enemy_pts = np.array(
                [self._hpbar_center(cb) for cb in enemy_creeps], dtype=np.float32
            )
        elif len(enemy_heroes) >= 1:
            enemy_pts = np.array(
                [self._hpbar_center(b) for b in enemy_heroes], dtype=np.float32
            )
        else:
        ...
        if not leaders_enemy_boxes:
            ...
            ally_leader_pts = np.array(
                [self._hpbar_center(b) for b in leaders_ally_boxes], dtype=np.float32
            )
            enemy_leader_pts = np.array(
                [self._hpbar_center(b) for b in leaders_enemy_boxes], dtype=np.float32
            )
            mid_lead, d_lead, n_lead, _, _ = self._compromise_line(
                ally_leader_pts, enemy_leader_pts
            )
    ...
    def _pick_nearest_lane_key(
        self, s: Senses, cur_uv: Tuple[float, float]
    ) -> Optional[str]:
        root = s.landmarks or {}
        best_key = None
        best_d2 = 1e18
        ...
        for key in ("lane_top", "lane_mid", "lane_bot"):
            arr = root.get(key)
            if not (
                isinstance(arr, list) and arr and isinstance(arr[0], list) and arr[0]
            ):
                continue
            poly = arr[0]
    ...
    def _build_lane_anchor_near_t1(
        self, c: Dict[str, Any], s: Senses, lane_key: str
    ) -> Optional[Tuple[float, float]]:
        """
    ...
    def _pick_next_ready_camp(
        self, c: Dict[str, Any], *, kind: Optional[CampKind] = None
    ) -> Optional[CampNode]:
        if not self._camps:
            return None
        cur = self._get_self_uv(c) or (50.0, 50.0)
    ...
    def _nearest_ally_hero_screen_xy(
        self, c: Dict[str, Any]
    ) -> Optional[Tuple[float, float]]:
        heroes = c.get("heroes", {})
        self_heroes = heroes.get("self", [])
    ...
    def _build_retreat_path_uv(
        ...
        *,
        ...
        for pt in ctrl:
            ...
            if _euclid2((sx, sy), (x, y)) < (
                self.retreat_path_arrive_radius_uv * self.retreat_path_arrive_radius_uv
            ):
                continue
    ...
    def _get_ally_creep_anchor_screen(
        self, c: Dict[str, Any], *, back_offset_px: float = 90.0
    ) -> Optional[Tuple[int, int]]:
        creeps = c.get("creeps", {})
        ally_creeps = creeps.get("ally", [])
        enemy_creeps = creeps.get("enemy", [])
    ...
    def _point_at_progress(
        self, poly: List[Dict[str, float]], t: float
    ) -> Optional[Tuple[float, float]]:
        if not poly:
            return None
    ...
    def _resolve_base_uv(
        self, c: Dict[str, Any], s: Senses, *, ally: bool
    ) -> Optional[Tuple[float, float]]:
        root = self._lane_landmarks_root(c, s)
    ...
    def _lane_is_reversed(
        self, c: Dict[str, Any], s: Senses, poly: List[Dict[str, float]]
    ) -> bool:
    ...
    def _build_lane_target_between_front_towers(
        ...
        ally_progress = self._front_tower_progresses(
            ally_towers, poly, lane_reversed=lane_reversed
        )
        enemy_progress = self._front_tower_progresses(
            enemy_towers, poly, lane_reversed=lane_reversed
        )
    ...
    def _set_state(self, new_state: BrainState):
        ...
        if self.log:
            self.log.info(
                f"[BRAIN {hex(self.hwnd)}] {self.state.name} -> {new_state.name}"
            )
    ...
    def _tick_farming_jungle(self, c: Dict[str, Any], s: Senses) -> None:
        ...
        if self._farm_phase is FarmPhase.FIGHT:
            ...
            if (
                self._farm_fight_started_near_target
                and self._farm_target_id is not None
            ):
        ...
        if self._farm_phase is FarmPhase.MOVE_TO_TARGET:
            ...
            if near_target_camp:
                ...
                if (
                    now - self._farm_arrived_at_camp_ts
                    >= self._farm_stuck_near_camp_timeout
                ):
                    if self.log:
                        self.log.info(
    ...
    def _tick_farming_lane(self, c: Dict[str, Any], s: Senses) -> None:
        ...
        if self._lane_farm_phase is LaneFarmPhase.SELECT_SEGMENT:
            ...
            if self._lane_key is None:
                ...
                target_uv = self._build_lane_target_between_front_towers(
                    c, s, self._lane_key
                )
        ...
        if self._lane_farm_phase is LaneFarmPhase.WAIT_OR_CLEAR_WAVE:
            if s.low_hp or s.near_enemy_tower:
                ...
                if s.enemy_creep_near:
                    ...
                    if self._lasthit_target_key is not None:
                        if self._lasthit_target_still_present(c):
                            ...
                            if enemy_creep_count <= 1 and now_ts >= (
                                self._lasthit_target_expire - 0.35
                            ):
                                self._lasthit_target_key = None
                    ...
                    lasthit_candidate = self._select_creep(
                        c, hp_threshold=0.6, enemy=True
                    )
                    if lasthit_candidate is not None:
                        cb, cx, cy = lasthit_candidate
                        ...
                        attacked = self._attack_on_screen_throttled(
                            cx, cy, y_offset=10, cooldown=0.25
                        )
                        if attacked:
                            self._lasthit_target_key = self._make_lasthit_key(
                                cb, cx, cy
                            )
                    ...
                    if s.ally_creep_near and enemy_creep_count > 1:
                        anchor = self._get_ally_creep_anchor_screen(
                            c, back_offset_px=90.0
                        )
                        if anchor is not None:
                            ax, ay = anchor
                            if s.enemy_hero_near:
                                self._walk_throttled(
                                    ax, ay, cooldown=0.45, tol_px=20.0, attack=False
                                )
                            else:
                                self._walk_throttled(
                                    ax, ay, cooldown=0.35, tol_px=20.0, attack=False
                                )
                    ...
                    if (
                        s.ally_creep_near
                        and not s.enemy_hero_near
                        and not s.near_enemy_tower
                    ):
                        anchor = self._get_ally_creep_anchor_screen(
                            c, back_offset_px=-10.0
                        )
                        if anchor is not None:
                            ax, ay = anchor
                            self._walk_throttled(
                    ...
                    if self._lane_wave_seen and (
                        now - self._lane_wave_last_seen_ts >= self.lane_wave_clear_grace
                    ):
                ...
                if waited >= self.lane_wave_timeout:
                    self._lane_farm_phase = LaneFarmPhase.DONE
                if (not self._lane_wave_seen) and (
                    waited >= self.lane_find_wave_timeout
                ):
                    self._lane_farm_phase = LaneFarmPhase.DONE
    ...
    def _tick_retreat(self, c: Dict[str, Any], s: Senses) -> None:
        ...
        if self._retreat_phase is RetreatPhase.GO_BEHIND_ALLY:
            ...
            if self._should_anchor_behind_ally(s):
                ...
                if ally_xy is not None:
                    enemy_xy = (
                        self._nearest_enemy_screen_xy(c) if enemy_on_screen else None
                    )
                    ...
                    if (
                        d2 <= (28.0 * 28.0)
                        or (now - self._retreat_phase_ts) >= self.retreat_anchor_timeout
                    ):
                        done = True
                else:
                    done = True
        ...
        if self._retreat_phase is RetreatPhase.BUILD_PATH:
            enemy_uv = (
                self._nearest_enemy_uv(c)
                if (enemy_on_minimap or threat_recent)
                else None
            )
        ...
        if not sent and (now - self._farming_last_minimap_click_ts) > 0.75:
            self.pl.click_minimap_pct(
                self.hwnd, fountain_uv[0], fountain_uv[1], attack=False
            )
            self._farming_last_minimap_click_ts = now
            self.last_action_ts = now
    ...
    def _tick_wait_start(self, c: Dict[str, Any], s: Senses):
        ...
        if self._dist2_uv(me_x, me_y, tx, ty) <= r2:
            if self.log:
                self.log.debug(
                    f"[BRAIN {hex(self.hwnd)}] WAIT_START: already near selected T1"
                )
            return
