...
BASE_URL = os.getenv("HOST_API_BASE_URL", "http://127.0.0.1:8000")
VM_ID = os.getenv("VM_ID", platform.node() or "vm-unknown")
SIDE = os.getenv("VM_SIDE", "radiant")
CAPACITY = int(os.getenv("VM_CAPACITY", "5"))
DEBUG = os.getenv("VM_DEBUG", "1").strip().lower() not in {"0", "false", "no"}
...
class VmClientController:
    ...
    def bootstrap(self) -> None:
        print(
            f"[VM] bootstrap config: vm_id={VM_ID} side={SIDE} capacity={CAPACITY} host={BASE_URL}",
            flush=True,
        )
        print("[VM] register_vm...", flush=True)
        while True:
            try:
                response = self.api.register_vm(capacity=CAPACITY, side=SIDE)
                print(f"[VM] register_vm <- {response}", flush=True)
                break
            except Exception as e:
                print(f"[VM] register_vm failed: {e}", flush=True)
                time.sleep(1.0)
